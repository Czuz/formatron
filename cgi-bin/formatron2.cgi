...
my @kmetal = ( '4B0082', 'ff9900', '8080f0', '7B68EE' ); #3cace4
my @kkrysztal = ( '4B0082', 'ff9900', '8080f0', '7B68EE' ); #3ce4ba
my @kdeuter = ( '4B0082', 'ff9900', '8080f0', '7B68EE' ); #e4c03c
...
my %nazwy_flota = (
	...
	"O.wojenny."	=> [ 6, "Okręt wojenny", 45000, 15000, 0,
	...
	"Panc."	=> [ 14, "Pancernik", 30000, 40000, 15000,
			"FF0000", "a099ff", "FF4500", "e43ca1" ]
);
my %nazwy_obrona = (
...
my $bitwa = $p{bitwa} || $ARGV[3] || "";
my $nagl = $p{nagl} || $ARGV[4] || 0;		#1 - pokazuje naglowek
my $czydata = $p{czydata} || $ARGV[5] || 0;	#1 - pokazuje date
my $czyczas = $p{czyczas} || $ARGV[6] || 0;	#1 - pokazuje godzine
my $podsumdeu = $p{podsumdeu} || $ARGV[7] || 0; #1 - liczy deuter do strat
my $podsumsur = $p{podsumsur} || $ARGV[8] || 0; #1 - rozdziela na surowce

my (@flota_przed, @flota_po, @straty);
my @gracz;
...
{
	my ($kto) = @_;
	my (%tmp,%tmp_x,%tmp_straty);
	my ($flota,$obrona)=("","");

	%tmp_straty=%{$flota_przed[$kto]};
...
my ($sam,$sak,$sad,$som,$sok,$sod) = (0,0,0,0,0,0);
my ($sm,$sk,$sd) = (0,0,0);
my $przedliczb='[b][color=#'.$wyroznienie[$paleta].']';
my $przedliczbm='[b][color=#'.$kmetal[$paleta].']';
my $przedliczbk='[b][color=#'.$kkrysztal[$paleta].']';
my $przedliczbd='[b][color=#'.$kdeuter[$paleta].']';
my $policzb='[/color][/b]';
my $zlomwlasny=0;
...
while ($_=$rw[0]) {
	...
	if (/^Agresor stracił/) {
		for(my $i=1; $i<=$#gracz; $i++) {
			my %tmp_straty = %{$straty[$i]};
			foreach my $klucz (keys %tmp_straty) {
				if (czy_agresor($i)) {
					$sam -=
						($nazwy_flota{$klucz} ?
						$nazwy_flota{$klucz}[2] :
					       	$nazwy_obrona{$klucz} ?
						$nazwy_obrona{$klucz}[2] :
					       	0)
					     	* $tmp_straty{$klucz};
					$sak -=
						($nazwy_flota{$klucz} ?
	...
	if (/^(Agresor|Obrońca) stracił/) {
		my ($strata, $stratam, $stratak, $stratad) = (0,0,0,0);
		if (/^Agresor/) {
			$strata = $sam+$sak;
			if ($podsumdeu) { $strata += $sad; }
			$stratam = $przedliczbm.commify($sam).$policzb;
			$stratak = $przedliczbk.commify($sak).$policzb;
			$stratad = $przedliczbd.commify($sad).$policzb;
			if ($kto_wygral == 1) {
				$zlomwlasny = $strata;
				$sm = $sam;
				$sk = $sak;
			...
			$strata = $som+$sok;
			if ($podsumdeu) { $strata += $sod; }
			$stratam = $przedliczbm.commify($som).$policzb;
			$stratak = $przedliczbk.commify($sok).$policzb;
			$stratad = $przedliczbd.commify($sod).$policzb;
			if ($kto_wygral == 2) {
				$zlomwlasny = $strata;
				$sm = $som;
				$sk = $sok;
		...
		s/\d+/$strata/;
		s/\./. ($stratam\/$stratak\/$stratad)/ if $podsumsur;
	}
...
print "\n\nTeoretyczny zysk zwycięzcy wynosi ",
	...
	$policzb,
	" jednostek." if $kto_wygral;
        if ($podsumsur and $kto_wygral) {
		print " (",
		$przedliczbm,
		commify($m+$zlom_metal-$sm),
		$policzb,
		"\/",
	        $przedliczbk,
        	commify($k+$zlom_krysia-$sk),
	        $policzb,
        	"\/",
	        $przedliczbd,
        	commify($d-$sd),
